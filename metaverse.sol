...
contract Metaverse is ERC721,Ownable {
    ...
    function mint(string memory _object_name,int8 _w,int8 _h,int8 _d,int8 _x,int8 _y,int8 _z) public payable{
        require(supply.current()<=maxSupply,"Supply exceeds maximum"); //check current supply is less or eqaual to maxSupply 
        require(msg.value>=cost,"Insufficient Payment");//check current value is less or eqaual to cost 
        supply.increment();
